...
<form method="POST" action="{{route('set_location')}}">
@csrf
...
<div id="mega_menu" @if(isset($language) && $language=='ar') style="text-align:right;" @endif>
    ...
    <ul id="c_list">
		...
		<a class="menu-links" href="#" id="">{{ $category_data->name }} @if (count($category_data->children))
			@if(isset($language) && $language=='ar')
		                        ...
		                        @if (count($category_data->children))
                                                            <ul  class="c_list_drop">
                                                                @foreach ($category_data->children as $sub)
                                                                    <li>
																		<a class="drop-links"  onmouseover="displaysubMenu(this,{{$sub['id']}})"
    ...
    <div id="subcategory-side" class="sides">
		@foreach($categories as $category_data)
		@foreach($category_data->children as $sub)
		<div class="sub-wrapper" id="{{$sub['id']}}">
		@foreach($sub->children as $sub_next)
        <div class="sub-container">
			<h5 class="sub-title"><a class="title-links" style="font-weight:bold;" href="{{ url('/'  . $category_data->slug . '/'  . $sub['slug'] . '/' . $sub_next['slug']) }}">{{$sub_next['name']}}</a></h5>
		@foreach($sub_next->children as $sub_last)
			<p class="sub-listing"><a class="listing-links" href="{{ url('/'  . $category_data->slug . '/'  . $sub['slug'] . '/' . $sub_next['slug']. '/' . $sub_last['slug']) }}">{{$sub_last['name']}}</a></p>	
		@endforeach
        </div>
		@endforeach
		</div>
                    ...
                    <div class="ps-top__right">
                        ...
                        <ul class="menu-top">
                            ...
                            <li class="nav-item"><a class="nav-link" target="_blank" href="/page/contact-us?locale=en">Contact</a></li>
							  {{--
                                 <li class="colors nav-item" data-color="#e74c3c"><i class="fa fa-x fa-moon-o" aria-hidden="true"></i></li>
                                 <li class="colors nav-item" data-color="#00c853"><i class="fa fa-star"></i></li>--}}
                        </ul>
        ...
        <div class="middle-bar">
            <div class="container">
                <div class="ps-header__middle">
                    <div class="container">
						<div class="ps-logo"><a id="menu_bars" onclick="displayMenu()"><i class="fa fa-bars fa-lg"></i></a><a href="/"> <img src="{{ asset('magic/assets/img/habib/logo1.png') }}" alt=""></a>
                        ...
                        <div class="ps-header__right">
                            <ul class="ps-header__icons">
                                ...
                                <li>@if(!Auth::guard('customer')->user()) <a href="{{ route('customer.session.create') }}" class="ps-header__item"><i class="icon-user"></i></a> @endif
									<style>
.dropbtn {
                                        ...
                                        @if ($cart)
                                            <ul class="ps-cart__items">
                                                @foreach ($cart->items as $item)
                                                    ...
                                                    ?>
                                                    <li class="ps-cart__item" id="item_{{ $item->id }}">
                                                        <div class="ps-product--mini-cart">
                                                            <a class="ps-product__thumbnail"
                                                                href="{{ route('shop.productOrCategory.index', $url_key) }}">
                                                                <img
                                                                    src="{{ $productBaseImage['original_image_url'] }}"alt="alt">
                                                            </a>
                                                            <div class="ps-product__content"><a class="ps-product__name"
                                                                    href="{{ route('shop.productOrCategory.index', $url_key) }}">{{ $item->name }}</a>
                                                                <p class="ps-product__meta"><span
                                                                        class="{{ $item->id }}">{{ $item->quantity }}</span>x
                                                                    <span
                                                                        class="ps-product__price">{{ core()->currency($item->price) }}</span>
                                                                </p>
                                            ...
                                            <div class="ps-cart__total"><span>Subtotal
                                                </span><span>{{ core()->currency($cart->base_grand_total) }}</span>
                                            </div>
                                            <div class="ps-cart__footer"><a class="ps-btn ps-btn--outline"
                                                    href="{{ route('shop.checkout.cart.index') }}">View Cart</a><a
                                                    class="ps-btn ps-btn--warning"
                                                    href="{{ route('shop.checkout.onepage.index') }}">Checkout</a>
                                            </div>
                                        @else
...
$locales=DB::table('locales')->select('locales.*')->get();?>
  <div class="dropdown-content">
    ...
    <a class="nav-link" href="?locale={{$locale->code}}">
          <img src="{{url('').'/storage/'.$locale->locale_image}}" style="max-width:20% !important;"/> {{$locale->name}}
        </a>
	  @endforeach
  </div>
</div>				 			 
                                ...
                                <form action="{{ url('search') }}">
                                    <div class="ps-search-table">
                                        <div class="input-group">
                                            <input class="form-control ps-input" type="text" id="action-search" name="term" value="{{ request()->get('term') }}" required
				...
				<!--Comment Here
                    <nav class="ps-main-menu">
                        <ul class="menu">
                            ...
                            @endif
                            {{-- <img class="img-fluid " src="{{ url('/' . 'storage/' . $category_data->image) }}"> --}}


                            @foreach ($categories as $category_data)
                                <li class="has-mega-menu">
                                    <a
                                        @if (count($category_data->children)>0) href="#" @else href="{{ url('/' . $item['slug']) }}" @endif>{{ $category_data->name }}<span
...
<header class="ps-header ps-header--2 ps-header--mobile">
            ...
            <div class="ps-nav__item"><a href="{{ route('customer.session.create') }}">
    ...
    <div class="ps-noti">
        ...
        <div class="ps-header__middle">
            <div class="container">
                <div class="ps-logo"><a href="#"> <img src="{{ asset('magic/assets/img/habib/logo1.png') }}" alt=""></a></div>
                <div class="ps-nav__item">
					<a href="{{ route('shop.checkout.cart.index') }}"><i class="icon-cart-empty"></i><span
                            class="badge">{{ $cartItemsCount }}</span> </a>
							<!--New-->
                ...
                <div class="ps-header__search">
                        ...
                        <div class="ps-search-table">
                            <div class="input-group">
                                <input class="form-control ps-input" type="text" id="action-search" name="term" value="{{ request()->get('term') }}" required placeholder="Search for products">
								<div class="input-group-append">
	...
	<div class="ps-menu--slidebar">
        <div class="ps-menu__content">
            <ul class="menu--mobile">
                            @foreach ($categories as $category_data)
                <li class="menu-item-has-children"><a @if (count($category_data->children)>0) href="#" @else href="{{ url('/' . $item['slug']) }}" @endif>{{ $category_data->name }}</a><span class="sub-toggle"><i class="fa fa-chevron-down"></i></span>
                                                    @if (count($category_data->children))
                    <ul class="sub-menu">
                                                            @foreach ($category_data->children as $sub)
                                                                <li><a
...
$("#form-submit").on('submit', function(e){
    ...
    $.ajax({
        type: "POST",
        url: "/set-location",
        data: { variable: distance, _token: '{{csrf_token()}}'  },
