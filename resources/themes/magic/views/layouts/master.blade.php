...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('magic/assets/fonts/Linearicons/Font/demo-files/demo.css') }}">
    <link rel="preconnect" href="https://fonts.gstatic.com">
    <link rel="stylesheet" href="../../css?family=Jost:400,500,600,700&amp;display=swap&amp;ver=1607580870">
    <link rel="stylesheet" href="{{ asset('magic/assets/plugins/bootstrap4/css/bootstrap.min.css') }}">
    <link rel="stylesheet" href="{{ asset('magic/assets/plugins/owl-carousel/assets/owl.carousel.css') }}">
    <link rel="stylesheet" href="{{ asset('magic/assets/plugins/slick/slick/slick.css') }}">
    <link rel="stylesheet" href="{{ asset('magic/assets/plugins/lightGallery/dist/css/lightgallery.min.css') }}">
    <link rel="stylesheet" href="{{ asset('magic/assets/plugins/jquery-bar-rating/dist/themes/fontawesome-stars.css') }}">
    <link rel="stylesheet" href="{{ asset('magic/assets/plugins/select2/dist/css/select2.min.css') }}">
    <link rel="stylesheet" href="{{ asset('magic/assets/plugins/lightGallery/dist/css/lightgallery.min.css') }}">
    <link rel="stylesheet" href="{{ asset('magic/assets/plugins/noUiSlider/nouislider.css') }}">
    <link rel="stylesheet" href="{{ asset('magic/assets/css/style.css') }}">
    <link rel="stylesheet" href="{{ asset('magic/assets/css/home-2.css') }}">
    <link rel="stylesheet" href="{{ asset('magic/assets/fonts/Linearicons/Font/flaticon.ttf') }}">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Raleway">
    <link rel="stylesheet" href="{{ asset('magic/assets/fonts/Linearicons/Font/flaticon.css') }}">
    @yield('head')
@push('styles')
	<style>
    ...
    @endphp         
    <script>	
        document.documentElement.style.setProperty('--color-green', '{{ $color->color_hex }}');
    </script>
</head>
...
<body @if (core()->getCurrentLocale() && core()->getCurrentLocale()->direction == 'rtl') class="rtl" @endif>
    ...
    <div id="app">
        ...
        <div class="main-container-wrapper">
            ...
            <main class="content-container"  @if($imagefront->image=='Null') @else style="scroll-behavior: smooth;background-image: url({{$imagefront->image}});" @endif>
    ...
    <script src="{{ asset('magic/assets/plugins/jquery.min.js') }}"></script>
    <script src="{{ asset('magic/assets/plugins/popper.min.js') }}"></script>
    <script src="{{ asset('magic/assets/plugins/bootstrap4/js/bootstrap.min.js') }}"></script>
    <script src="{{ asset('magic/assets/plugins/select2/dist/js/select2.full.min.js') }}"></script>
    <script src="{{ asset('magic/assets/plugins/owl-carousel/owl.carousel.min.js') }}"></script>
    <script src="{{ asset('magic/assets/plugins/jquery-bar-rating/dist/jquery.barrating.min.js') }}"></script>
    <script src="{{ asset('magic/assets/plugins/lightGallery/dist/js/lightgallery-all.min.js') }}"></script>
    <script src="{{ asset('magic/assets/plugins/slick/slick/slick.min.js') }}"></script>
    <script src="{{ asset('magic/assets/plugins/noUiSlider/nouislider.min.js') }}"></script>
    <!-- custom code-->
    <script src="{{ asset('magic/assets/js/main.js') }}"></script>
    ...
    <script>
        function addToCart(id, type) {
            ...
            if (type == 'simple') {
                ...
                console.log(data);
                    ...
                    toastMixin.fire({
                        animation: true,
                        icon: 'error',
                        title: @json(trans('shop::app.checkout.cart.quantity.inventory_warning'))
                    });
                }
            ...
            if (!data) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ e(url()->to('/')) . '/api/checkout/cart/add/' }}" + id,
                    data: data,
                        ...
                        if ($('.mini_cart_inner').length) {
                            ...
                            var mini_cart_footer = `<div class="dropdown-cart-total">
                                                    ...
                                                    <div class="dropdown-cart-action">
                                                        <a href="{{ route('shop.checkout.cart.index') }}" class="btn btn-primary">View
                                                            Cart</a>
        ...
        function removeProduct(id) {
            ...
            $.ajax({
                type: "GET",
                url: "{{ e(url()->to('/')) . '/api/checkout/cart/remove-item/' }}" + id,
                data: {},
                success: function(result) {
                    console.log(result);
                    if (result['data']) {
        ...
        $("#btnlogin").click(function(event) {
            ...
            if (email == "" || password == "") {
                ...
                $.ajax({
                    type: "Post",
                    url: "{{ e(url()->to('/')) . '/api/customer/login' }}",
                    data: {
                        email: email,
                        password: password
                    },
                    success: function(result) {
                        ...
                        if (result['success'] == true) {
                            window.location.replace(
                                "{{ e(url()->to('/')) . '/customer/account/profile' }}");
                        } else {
                            document.getElementById("err").style.display = "initial";
        ...
        $("#btnregister").click(function(event) {
            ...
            if (fn == "" || ln == "" || phone == "" || email == "" || country == "" || password == "" || confpass ==
                ...
                $.ajax({
                    type: "Post",
                    url: "{{ e(url()->to('/')) . '/api/customer/register' }}",
                    data: {
                        first_name: fn,
                        last_name: ln,
                        phone: phone,
